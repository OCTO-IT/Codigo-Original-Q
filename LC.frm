...
PosicionInicialAlturaCliente=599
...
ExpresionesAlMostrar=Asigna(Info.LineaCredito, Nulo)<BR>Asigna(Info.Cliente, Nulo)<BR>Asigna(Info.Proveedor, Nulo)<BR>Asigna(Info.Uso, Nulo)
...
FiltroGeneral=LC.Express=0 AND <BR>{Si(Vacio(Usuario.ACLCUsoEspecifico), <T>1=1<T>, <T>LC.Uso=<T>+Comillas(Usuario.ACLCUsoEspecifico))}
...
Visible=S
Antes=S
ActivoCondicion=LC:LC.Uso=<T>Fondeo<T>
AntesExpresiones=Asigna(Info.LineaCredito, LC:LC.LineaCredito)
...
(Inicio)=LC:LC.LineaCredito
LC:LC.LineaCredito=LC:LC.Estatus
LC:LC.Estatus=LC:LC.Alta
LC:LC.Alta=LC:LC.Situacion
LC:LC.Situacion=LC:LC.SituacionFecha
LC:LC.SituacionFecha=LC:LC.Contrato
LC:LC.Contrato=(Fin)
...
FiltroGeneral=SituacionCta.Rama=<T>AC<T> AND SituacionCta.Cuenta=<T>{LC:LC.LineaCredito}<T>
