...
EnMenu=S
TipoAccion=Expresion
Expresion=Asigna(Info.Tabla, <T>Usuario<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>))
Activo=S
ConCondicion=S
EjecucionCondicion=ConDatos(Usuario:Usuario.Usuario)
Antes=S
...
CondicionVisible=General.ProdInterfazINFOR Y 1=2
...
Expresion=EjecutarSQL(<T>spCopiarUsuarioAcceso :tAcceso,:tUsuario<T>,Usuario:Usuario.Acceso,Usuario:Usuario.Usuario )<BR>
