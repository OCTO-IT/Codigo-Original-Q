[Forma]
Clave=SocioInfo
Nombre=Información del Socio
Icono=0
Modulos=(Todos)
ListaCarpetas=(Lista)
CarpetaPrincipal=Ficha
PosicionInicialIzquierda=203
PosicionInicialArriba=166
PosicionInicialAlturaCliente=348
PosicionInicialAncho=742
PosicionColumna1=43
BarraHerramientas=S
AccionesTamanoBoton=15x5
AccionesDerecha=S
ListaAcciones=(Lista)
Comentarios=Info.Socio

[Capital]
Estilo=Iconos
Clave=Capital
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=A2
Vista=CapitalT
Fuente={Tahoma, 8, Negro, []}
CampoColorLetras=Negro
CampoColorFondo=Blanco
CarpetaVisible=S
IconosCampo=(sin Icono)
IconosEstilo=Detalles
IconosAlineacion=de Arriba hacia Abajo
IconosConSenales=S
ElementosPorPagina=200
IconosSubTitulo=<T>Movimiento<T>
ListaEnCaptura=(Lista)
Pestana=S
PestanaOtroNombre=S
PestanaNombre=Capital
Filtros=S
MenuLocal=S
FiltroPredefinido=S
FiltroNullNombre=(sin clasificar)
FiltroEnOrden=S
FiltroTodoNombre=(Todo)
FiltroAncho=20
FiltroRespetar=S
FiltroTipo=General
ListaAcciones=(Lista)
IconosNombre=CapitalT:Capital.Mov+<T> <T>+CapitalT:Capital.MovID
FiltroGeneral=CapitalD.Socio=<T>{info.Socio}<T>

[Capital.Columnas]
0=137
1=79

[Acciones.Cerrar]
Nombre=Cerrar
Boton=23
NombreEnBoton=S
NombreDesplegar=&Cerrar
EnBarraHerramientas=S
TipoAccion=Ventana
ClaveAccion=Cerrar
Activo=S
Visible=S

[Ficha]
Estilo=Ficha
Pestana=S
PestanaOtroNombre=S
PestanaNombre=Datos Generales
Clave=Ficha
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=A1
Vista=Socio
Fuente={Tahoma, 8, Gris, []}
FichaEspacioEntreLineas=6
FichaEspacioNombres=100
FichaEspacioNombresAuto=S
FichaNombres=Arriba
FichaAlineacion=Izquierda
FichaColorFondo=Plata
FichaAlineacionDerecha=S
CampoColorLetras=Negro
CampoColorFondo=Plata
ListaEnCaptura=(Lista)
CarpetaVisible=S
Filtros=S
FiltroPredefinido=S
FiltroNullNombre=(sin clasificar)
FiltroEnOrden=S
FiltroTodoNombre=(Todo)
FiltroAncho=20
FiltroGeneral=Socio.Socio=<T>{info.socio}<T>
FiltroRespetar=S
FiltroTipo=General

[Ficha.Socio.Socio]
Carpeta=Ficha
Clave=Socio.Socio
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Nombre]
Carpeta=Ficha
Clave=Socio.Nombre
ValidaNombre=S
3D=S
Tamano=30
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Direccion]
Carpeta=Ficha
Clave=Socio.Direccion
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=41
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Colonia]
Carpeta=Ficha
Clave=Socio.Colonia
Editar=S
LineaNueva=S
3D=S
Tamano=10
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.CodigoPostal]
Carpeta=Ficha
Clave=Socio.CodigoPostal
Editar=S
3D=S
Pegado=S
Tamano=8
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Estado]
Carpeta=Ficha
Clave=Socio.Estado
Editar=S
3D=S
Tamano=10
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Pais]
Carpeta=Ficha
Clave=Socio.Pais
Editar=S
3D=S
Pegado=S
Tamano=9
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Telefonos]
Carpeta=Ficha
Clave=Socio.Telefonos
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=30
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Extencion]
Carpeta=Ficha
Clave=Socio.Extencion
Editar=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Grupo]
Carpeta=Ficha
Clave=Socio.Grupo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Estatus]
Carpeta=Ficha
Clave=Socio.Estatus
Editar=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Categoria]
Carpeta=Ficha
Clave=Socio.Categoria
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.Familia]
Carpeta=Ficha
Clave=Socio.Familia
Editar=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.RFC]
Carpeta=Ficha
Clave=Socio.RFC
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Ficha.Socio.CURP]
Carpeta=Ficha
Clave=Socio.CURP
Editar=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Plata
ColorFuente=Negro

[Capital.Capital.FechaEmision]
Carpeta=Capital
Clave=Capital.FechaEmision
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco
ColorFuente=Negro

[Capital.Capital.Estatus]
Carpeta=Capital
Clave=Capital.Estatus
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=15
ColorFondo=Blanco
ColorFuente=Negro

[Acciones.Tareas]
Nombre=Tareas
Boton=70
NombreDesplegar=Tareas
EnBarraHerramientas=S
TipoAccion=Formas
Activo=S
Visible=S
EspacioPrevio=S
ClaveAccion=Tarea
Antes=S
ConCondicion=S
EjecucionCondicion=ConDatos( Socio:Socio.Socio)
AntesExpresiones=Asigna(Info.PuedeEditar, Verdadero)<BR>Asigna(Info.Nombre, Socio:Socio.Nombre)<BR>Asigna(Info.Cliente, Nulo)<BR>Asigna(Info.Proveedor, Nulo)<BR>Asigna(Info.Socio, Socio:Socio.Socio)<BR>Asigna(Info.Personal, Nulo)<BR>Asigna(Info.Proyecto, Nulo)<BR>Asigna(Info.UEN, Nulo)<BR>Asigna(Info.Modulo, Nulo)<BR>Asigna(Info.ID, Nulo)<BR>Asigna(Info.Reporte, Nulo)

[Acciones.Propiedades]
Nombre=Propiedades
Boton=35
NombreDesplegar=Propiedades
EnBarraHerramientas=S
TipoAccion=Formas
ClaveAccion=propInfo
Activo=S
Visible=S
Antes=S
ConCondicion=S
EjecucionCondicion=ConDatos( Socio:Socio.Socio)
AntesExpresiones=Asigna(Info.Rama, <T>CAP<T>)<BR>Asigna(Info.Cuenta,Socio:Socio.Socio)<BR>Asigna(Info.Descripcion, Socio:Socio.Nombre)

[Acciones.Evaluaciones]
Nombre=Evaluaciones
Boton=103
NombreDesplegar=Evaluaciones
EnBarraHerramientas=S
TipoAccion=Formas
ClaveAccion=EvaluacionCalificacionInfo
Activo=S
Antes=S
Visible=S
ConCondicion=S
EjecucionCondicion=ConDatos( Socio:Socio.Socio)
AntesExpresiones=Asigna(Info.Modulo, Nulo)<BR>Asigna(Info.Mov, Nulo)<BR>Asigna(Info.Clave, Socio:Socio.Socio)<BR>Asigna(Info.Nombre, Socio:Socio.Nombre)<BR>Asigna(Info.Aplica, <T>Socios<T>)<BR>Asigna(Info.Nombre2, Nulo)

[Acciones.Anexos]
Nombre=Anexos
Boton=77
NombreDesplegar=Ane&xos
EnBarraHerramientas=S
TipoAccion=Formas
ClaveAccion=AnexoCta
Activo=S
Visible=S
Antes=S
ConCondicion=S
EjecucionCondicion=ConDatos( Socio:Socio.Socio)
AntesExpresiones=Asigna(Info.Rama, <T>CAP<T>)<BR>Asigna(Info.AnexoCfg, Verdadero)<BR>Asigna(Info.Cuenta, Socio:Socio.Socio)<BR>Asigna(Info.Descripcion, Socio:Socio.Nombre)

[Acciones.Doc]
Nombre=Doc
Boton=17
NombreDesplegar=&Documentacion
EnBarraHerramientas=S
TipoAccion=Expresion
Activo=S
ConCondicion=S
Visible=S
Expresion=EditarDocumentacion(<T>CAP<T>, Socio:Socio.Socio, <T>Documentación - <T>+Socio:Socio.Nombre)
EjecucionCondicion=ConDatos( Socio:Socio.Socio)

[Acciones.Politicas]
Nombre=Politicas
Boton=22
NombreDesplegar=Politicas
EnBarraHerramientas=S
TipoAccion=Formas
ClaveAccion=PoliticaInfo
Activo=S
ConCondicion=S
Antes=S
Visible=S
EjecucionCondicion=ConDatos( Socio:Socio.Socio)
AntesExpresiones=Asigna(Info.Rama, <T>CAP<T>)<BR>Asigna(Info.Clave, Socio:Socio.Socio)

[Acciones.MovProp]
Nombre=MovProp
Boton=0
NombreDesplegar=MovProp
EnMenu=S
TipoAccion=Formas
Activo=S
Visible=S
ClaveAccion=MovPropiedades
ConCondicion=S
EjecucionCondicion=ConDatos( CapitalT:CapitalD.ID)
Antes=S
AntesExpresiones=Asigna(Info.Modulo, <T>CAP<T>)<BR>Asigna(Info.ID, CapitalT:CapitalD.ID)

[Acciones.CapitalPersonalizar]
Nombre=CapitalPersonalizar
Boton=0
NombreDesplegar=Personalizar Vista
EnMenu=S
Carpeta=Capital
TipoAccion=Controles Captura
ClaveAccion=Mostrar Campos
Activo=S
Visible=S
EspacioPrevio=S

[Forma.ListaCarpetas]
(Inicio)=Ficha
Ficha=Capital
Capital=(Fin)

[Forma.ListaAcciones]
(Inicio)=Cerrar
Cerrar=Tareas
Tareas=Propiedades
Propiedades=Evaluaciones
Evaluaciones=Anexos
Anexos=Doc
Doc=Politicas
Politicas=(Fin)

[Capital.ListaEnCaptura]
(Inicio)=Capital.FechaEmision
Capital.FechaEmision=Capital.Estatus
Capital.Estatus=(Fin)

[Capital.ListaAcciones]
(Inicio)=MovProp
MovProp=CapitalPersonalizar
CapitalPersonalizar=(Fin)

[Ficha.ListaEnCaptura]
(Inicio)=Socio.Socio
Socio.Socio=Socio.Nombre
Socio.Nombre=Socio.Direccion
Socio.Direccion=Socio.Colonia
Socio.Colonia=Socio.CodigoPostal
Socio.CodigoPostal=Socio.Estado
Socio.Estado=Socio.Pais
Socio.Pais=Socio.Telefonos
Socio.Telefonos=Socio.Extencion
Socio.Extencion=Socio.Grupo
Socio.Grupo=Socio.Estatus
Socio.Estatus=Socio.Categoria
Socio.Categoria=Socio.Familia
Socio.Familia=Socio.RFC
Socio.RFC=Socio.CURP
Socio.CURP=(Fin)
