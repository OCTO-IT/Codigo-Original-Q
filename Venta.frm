...
Expresion=Asigna(Info.ID,Venta:Venta.ID)<BR>Asigna(Info.Descripcion, SQL(<T>EXEC spAbrirAnexo :tModulo, :nID<T>, <T>VTAS<T>, Venta:Venta.ID))<BR>Si (Info.Descripcion <> NULO) Y (Archivo.Leer(Info.Descripcion) <> <T><T>)<BR>Entonces<BR>  VerImagen(<T><T>, Info.Descripcion)<BR>Sino<BR>  ReporteImpresora(ReporteMovImpresora(Info.SubModuloVenta, Venta:Venta.Mov, Venta:Venta.Estatus), Venta:Venta.ID)<BR>Fin
...
Expresion=Asigna(Info.ID,Venta:Venta.ID)<BR>Asigna(Info.Descripcion, SQL(<T>EXEC spAbrirAnexo :tModulo, :nID<T>, <T>VTAS<T>, Venta:Venta.ID))<BR>Si (Info.Descripcion <> NULO) Y (Archivo.Leer(Info.Descripcion) <> <T><T>)<BR>Entonces<BR>  VerImagen(<T><T>, Info.Descripcion)<BR>Sino<BR>  ReportePantalla(ReporteMovPantalla(Info.SubModuloVenta, Venta:Venta.Mov, Venta:Venta.Estatus), Venta:Venta.ID)<BR>Fin
...
Boton=64
Menu=&Edición
NombreDesplegar=Si(MovTipoEn(<T>VTAS<T>, Venta:Venta.Mov, (VTAS.VP, VTAS.SD, VTAS.B)), <T>Devolución...<T>, <T>Cobro...<T>)
GuardarAntes=S
RefrescarDespues=S
EnMenu=S
TipoAccion=Formas
...
EjecucionMensaje=Si(Vacio(Venta:Venta.ID), <T>Es Necesario Guardar el Movimiento Antes de Ejecutar esta Opción<T>, <T>Falta Indicar el Artículo<T>)
...
EjecucionCondicion=Si <BR>  Config.CancelarFactura=<T>Cambio Mes<T><BR>Entonces<BR>  Asigna(Temp.Fecha, SQL(<T>SELECT GETDATE()<T>))<BR>  Si<BR>    (Mes(Temp.Fecha)>Mes(Venta:Venta.FechaEmision)) o (Año(Temp.Fecha)>Año(Venta:Venta.FechaEmision))<BR>  Entonces<BR>    Falso<BR>  Sino<BR>    Verdadero<BR>  Fin<BR>Sino<BR>  Verdadero<BR>Fin
...
Expresion=Si<BR>  VentaD:VentaD.ExcluirISAN<BR>Entonces<BR>   Si(Confirmacion(<T>¿ Desea Incluir de Nuevo Este Accesorio en el Cálculo del ISAN ?<T>, BotonSi, BotonNo)=BotonSi, Asigna(VentaD:VentaD.ExcluirISAN, Falso))<BR>Sino<BR>   Si(Precaucion(<T>¿ Desea Excluir Este Accesorio en el Cálculo del ISAN ?<T>, BotonSi, BotonNo)=BotonSi, Asigna(VentaD:VentaD.ExcluirISAN, Verdadero))<BR>Fin
...
Nombre=VentaDWeb
Boton=0
NombreDesplegar=Detalles de Artículo
EnMenu=S
TipoAccion=Expresion
Activo=S
...
Activo=S
Antes=S
AntesExpresiones=ASIGNA(Info.ID,Venta:Venta.ID)<BR>ASIGNA(Info.Modulo,<T>VTAS<T>)
Visible=S
...
[Ficha.Venta.SVEsAnulado]
Carpeta=Ficha
Clave=Venta.SVEsAnulado
Editar=S
ValidaNombre=S
3D=S
ColorFondo=Blanco
...
[Ficha.Venta.TarjetaAmericasClub]
Carpeta=Ficha
Clave=Venta.TarjetaAmericasClub
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
Venta.Directo=Venta.SVEsAnulado
Venta.SVEsAnulado=Venta.TarjetaAmericasClub
Venta.TarjetaAmericasClub=(Fin)
...
[Acciones.fmxUsarTarjeta]
Nombre=fmxUsarTarjeta
Boton=87
NombreEnBoton=S
NombreDesplegar=America Club
EnBarraHerramientas=S
EnMenu=S
TipoAccion=Dialogos
ClaveAccion=fmxUsarTarjeta
Activo=S
Visible=S
...
[Acciones.fmxUsarTarjeta.fmxUsarTarjeta]
Nombre=fmxUsarTarjeta
Boton=0
TipoAccion=Dialogos
ClaveAccion=fmxUsarTarjeta
...
[Acciones.fmxUsarTarjeta.Expresion]
Nombre=Expresion
Boton=0
TipoAccion=Expresion
Activo=S
...
[Acciones.fmxUsarTarjeta.ListaAccionesMultiples]
(Inicio)=Expresion
Expresion=fmxUsarTarjeta
fmxUsarTarjeta=(Fin)
...
Anterior=Siguiente
Siguiente=fmxUsarTarjeta
fmxUsarTarjeta=Catalogos
...
ContParalelaMovDato=SituacionHistorico
SituacionHistorico=(Fin)
