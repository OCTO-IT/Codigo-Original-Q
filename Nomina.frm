...
Activo=S
ConCondicion=S
EjecucionCondicion=Si(General.MovConcurrente y ConDatos(Nomina:Nomina.ID) y (Nomina:Nomina.UltimoCambio<>SQL(<T>spMovInfoVerUltimoCambio :nID, :tModulo<T>, Nomina:Nomina.ID, <T>NOM<T>)), Dialogo(<T>MovConcurrente<T>) AbortarOperacion)<BR>Verdadero
...
Expresion=Asigna(Afectar.ID, Nomina:Nomina.ID)<BR>Asigna(Afectar.Mov, Nomina:Nomina.Mov)<BR>Asigna(Afectar.Fecha, Nomina:Nomina.FechaOrigen)<BR>Asigna(Info.Proyecto, Nomina:Nomina.Proyecto)<BR>Si<BR>  MovTipoEn(<T>NOM<T>, Nomina:Nomina.Mov, (NOM.N, NOM.NE))<BR>Entonces<BR>  Si <BR>    Nomina:Nomina.Estatus en (EstatusSinAfectar, EstatusPorConfirmar)<BR>  Entonces<BR>    Forma(<T>NominaGenerar<T>)<BR>    EjecutarSQL(<T>EXEC spNominaGenerarCampoExtra :nID, :tProyecto<T>, Afectar.ID, Info.Proyecto)<BR>    AbortarOperacion<BR>  Sino<BR>    Si(Confirmacion(<T>¿ Esta Seguro que Desea Afectar la Nomina ?<T>,BotonSi, BotonNo)=BotonNo, AbortarOperacion)<BR>  Fin<BR>Fin<BR>Afectar(<T>NOM<T>, Nomina:Nomina.ID, Nomina:Nomina.Mov, Nomina:Nomina.MovID, <T>Todo<T>, <T><T>, <T>Nomina<T>)
...
ActivoCondicion=Vacio(Nomina:Nomina.MovID) y (Nomina:Nomina.Estatus en (EstatusSinAfectar, EstatusPorConfirmar, EstatusBorrador)) y PuedeAfectar(Verdadero, Usuario.ModificarOtrosMovs, Nomina:Nomina.Usuario)y(no(Nomina:Nomina.NOI))
EjecucionCondicion=Vacio(SQL(<T>SELECT MovID FROM Nomina WHERE ID=:nID<T>, Nomina:Nomina.ID))
...
ActivoCondicion=PuedeAfectar(Usuario.Cancelar, Usuario.CancelarOtrosMovs, Nomina:Nomina.Usuario) y ConDatos(Nomina:Nomina.ID) y ConDatos(Nomina:Nomina.MovID) y<BR>(Nomina:Nomina.Estatus en (EstatusSinAfectar, EstatusPorConfirmar, EstatusSincro, EstatusBorrador, EstatusPendiente, EstatusConcluido, EstatusProcesar, EstatusVigente))o(Nomina:Nomina.NOI)
...
ActivoCondicion=Usuario.EnviarExcel y (Nomina:Nomina.Estatus en (EstatusSinAfectar, EstatusPorConfirmar, EstatusBorrador))
Visible=S
...
NominaD.Activo=NominaD.ClavePresupuestal
...
[Detalle.NominaD.ClavePresupuestal]
Carpeta=Detalle
