...
FiltroGeneral=ActivoF.Empresa=<T>{Empresa}<T><BR>{Si(ConDatos(Info.Articulo), <T> AND ActivoF.Articulo=<T>+Comillas(Info.Articulo), <T><T>)}<BR>AND ActivoF.Estado=<T>Alquilado<T>
