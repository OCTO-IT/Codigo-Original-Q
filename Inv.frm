...
DespuesGuardar=S
Visible=S
Expresion=Si <BR>  InvD:Art.Tipo=TipoVIN<BR>Entonces<BR>  Si(Inv:Inv.Estatus en (EstatusSinAfectar, EstatusPorConfirmar), Forma(<T>VINMovMultiple<T>), Forma(<T>VINMovMultipleInfo<T>))<BR>Sino<BR>  Asigna(Info.SumaSerieLote, Nulo)<BR>  Asigna(Info.Articulo, InvD:InvD.Articulo)<BR>  Si(Inv:Inv.Estatus en (EstatusSinAfectar, EstatusPorConfirmar), Forma(<T>SerieLoteMov<T>), Si(Usuario.ModificarSerieLoteProp, Forma(<T>SerieLoteMovProp<T>), Forma(<T>SerieLoteMovInfo<T>)))<BR>  Si<BR>    (no Config.MultiUnidades) y ConDatos(Info.SumaSerieLote) y (Info.SumaSerieLote<>ABS(InvD:InvD.Cantidad)) y (Inv:Inv.Estatus en (EstatusSinAfectar, EstatusPorConfirmar))<BR>  Entonces<BR>    Si<BR>      Confirmacion(<T>La Cantidad Difiere con la Selección.<T>+NuevaLinea+NuevaLinea+<T>¿ Desea Actualizar la Cantidad ?<T>,BotonSi, BotonNo)=BotonSi)<BR>    Entonces<BR>      Asigna(InvD:InvD.Cantidad, Si(Vacio(InvD:InvD.Cantidad, 0.0)<0,-Info.SumaSerieLote, Info.SumaSerieLote))<BR>    Fin<BR>  Fin<BR>Fin
ActivoCondicion=MovTipo(<T>INV<T>, Inv:Inv.Mov) noen (INV.OT, INV.OI, INV.SM, INV.SOL)) y Config.SeriesLotesMayoreo
...
ActivoCondicion=InvD:Art.ControlSalidasEntradas
AntesExpresiones=ASIGNA(Info.ID,Inv:Inv.ID)<BR>ASIGNA(Info.RenglonID,InvD:InvD.RenglonID)<BR>ASIGNA(Info.Articulo,InvD:InvD.Articulo)<BR>ASIGNA(Info.Cantidad,InvD:InvD.Cantidad)<BR>ASIGNA(Info.Modulo,<T>INV<T>)<BR>ASIGNA(Info.Concepto,Inv:Inv.Concepto)
...
InvD.PosicionDestino=InvD.Tarima
InvD.Tarima=InvD.ContUso
...
InvD.CantidadA=InvD.PosicionActual
InvD.PosicionActual=InvD.PosicionReal
InvD.PosicionReal=(Fin)
