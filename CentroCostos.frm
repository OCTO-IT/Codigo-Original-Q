...
EnMenu=S
TipoAccion=Expresion
Expresion=Asigna(Info.Tabla, <T>CentroCostos<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Lista<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>))
Activo=S
ConCondicion=S
EjecucionCondicion=ConDatos(CentroCostos:CentroCostos.CentroCostos)
Antes=S
