...
EspacioPrevio=S
ConCondicion=S
Expresion=Asigna(Info.ID, Campana:Campana.ID)<BR>Asigna(Temp.Reg, SQL(<T>SELECT ParaProspectos, ParaClientes, ParaProveedores, ParaPersonal, ParaAgentes FROM CampanaTipo WHERE CampanaTipo=:tCampanaTipo<T>, Campana:Campana.CampanaTipo))<BR>Dialogo(<T>CampanaAgregarContactos<T>)
ActivoCondicion=Campana:Campana.Estatus en (EstatusSinAfectar, EstatusBorrador, EstatusPorConfirmar, EstatusPendiente)
