...
FiltroMonedas=S
FiltroFechasCampo=GastoT.Vencimiento
...
FiltroGeneral=GastoT.Empresa=<T>{Empresa}<T><BR>{Si(ConDatos(Info.Proveedor), <T> AND GastoT.Acreedor=<T>+Comillas(Info.Proveedor), <T><T>)}
[Acciones.Cerrar]
Nombre=Cerrar
Boton=23
NombreEnBoton=S
