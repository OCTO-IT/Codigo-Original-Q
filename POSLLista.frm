...
FiltroGeneral=POSL.Empresa = <T>{Empresa}<T><BR>AND POSL.Sucursal = <T>{Sucursal}<T><BR>AND POSL.Estatus IN (<T>CONCLUIDO<T>, <T>TRASPASADO<T>)<BR>AND MovTipo.Clave NOT IN(<T>POS.CTCAC<T>,<T>POS.CTCRC<T>,<T>POS.STE<T>,<T>POS.FTE<T>,<T>POS.ET<T>)<BR>AND MovTipo.Modulo = <T>POS<T>
