...
Nombre=Asignar Actividades Pendientes al Recurso
Icono=29
Modulos=(Todos)
ListaCarpetas=Lista
...
Comentarios=Lista(Info.Proyecto, Info.Recurso, Info.Nombre, Si(Ver.Todo, <T>Todas las Actividades<T>, <T>Actividades sin Asignar<T>))
VentanaExclusiva=S
...
Visible=S
EspacioPrevio=S
Expresion=RegistrarSeleccion(<T>Lista<T>)<BR>EjecutarSQL(<T>spProyectoDAsignar :ID, :nEstacion, :tRecurso<T>, Info.ID, EstacionTrabajo, Info.Recurso)<BR>Forma.ActualizarForma
...
Boton=40
NombreEnBoton=S
NombreDesplegar=&Desasginar Selección
EnBarraHerramientas=S
TipoAccion=Expresion
Activo=S
Visible=S
...
EspacioPrevio=S
Expresion=RegistrarSeleccion(<T>Lista<T>)<BR>EjecutarSQL(<T>spProyectoDAsignar :ID, :nEstacion, :tRecurso, :tAccion<T>, Info.ID, EstacionTrabajo, Info.Recurso, <T>DESASIGNAR<T>)<BR>Forma.ActualizarForma
