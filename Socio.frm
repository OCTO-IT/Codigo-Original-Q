...
FiltroSucursales=N
IconosNombre=Socio:Socio.Socio
...
ConCondicion=S
Visible=S
Expresion=Asigna(Info.Tabla, <T>Socio<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>))
EjecucionCondicion=ConDatos(Socio:Socio.Socio)
Antes=S
