...
ConCondicion=S
Visible=S
Expresion=Asigna(Info.Tabla, <T>Personal<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>))
EjecucionCondicion=ConDatos(Personal:Personal.Personal)
Antes=S
...
Personal.ActividadMedicion=(Fin)
