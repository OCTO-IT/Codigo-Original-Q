...
TeclaRapida=Ctrl+H
ConCondicion=S
Expresion=Asigna(Info.Tabla, <T>VIN<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>),Forma(<T>VINHist<T>))
EjecucionCondicion=ConDatos(VIN:VIN.VIN)
