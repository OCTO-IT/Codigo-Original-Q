...
ActivoCondicion=PuedeAfectar(Usuario.Cancelar, Usuario.CancelarOtrosMovs, TMA:TMA.Usuario) y ConDatos(TMA:TMA.ID) y ConDatos(TMA:TMA.MovID) y<BR>((TMA:TMA.Estatus en (EstatusSinAfectar, EstatusBorrador, EstatusSincro, EstatusPendiente)) y (TMA:MovTipo.Clave <> <T>TMA.RADO<T>) /*y (TMA:MovTipo.Clave <> <T>TMA.ORADO<T>)*/<BR>o (TMA:TMA.Estatus en (EstatusConcluido, EstatusPorConfirmar, EstatusProcesar))) y (TMA:MovTipo.Clave <> <T>TMA.RADO<T>) /*y (TMA:MovTipo.Clave <> <T>TMA.ORADO<T>)*/
...
AntesExpresiones=Asigna(Info.Articulo, TMAD:Articulo)<BR>Asigna(Info.Empresa, TMA:TMA.Empresa)<BR>Asigna(Info.Almacen, TMAD:TMAD.Almacen)<BR>Asigna(Info.Tarima, TMAD:TMAD.Tarima)<BR>//Asigna(Info.Articulo, SQL(<T>SELECT Articulo FROM ArtDisponibleTarima WHERE Tarima = :tTarima AND Almacen = :tAlmacen AND Empresa = :tEmpresa<T>,Info.Tarima, Info.Almacen, Info.Empresa))
...
TMAD.Zona=Zona1
Zona1=TMAD.CantidadA
TMAD.CantidadA=TMAD.CantidadPicking
TMAD.CantidadPicking=Cantidad1
Cantidad1=CantidadPendiente
CantidadPendiente=TMAD.Unidad
TMAD.Unidad=TMAD.CantidadUnidad
TMAD.CantidadUnidad=TMAD.Prioridad
