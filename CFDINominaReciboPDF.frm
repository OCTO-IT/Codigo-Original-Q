...
Antes=S
Expresion=Si<BR>  sql(<T>SELECT EnviarPDFNomina FROM EMPRESACFD WHERE Empresa=:tEmpresa<T>,EMPRESA)<BR>Entonces<BR>  Si<BR>    CFDINominaRecibo:CFDINominaRecibo.PDFGenerado<BR>    Entonces<BR>        EjecutarSQL(<T>EXEC spEnviarReciboNomina :tEmpresa,:nID,:tPersonal,:tAcceso<T>,Empresa, CFDINominaRecibo:CFDINominaRecibo.RID,CFDINominaRecibo:CFDINominaRecibo.Personal,EstacionTrabajo)<BR><BR>    Sino<BR>     Falso<BR>      Error( <T>Error al enviar al Personal: <T>+ CFDINominaRecibo:CFDINominaRecibo.Personal +<T>, falta generar PDF<T> )<BR>    Fin<BR>Sino<BR>  EjecutarSQL(<T>EXEC spEnviarReciboNomina :tEmpresa,:nID,:tPersonal,:tAcceso<T>,Empresa, CFDINominaRecibo:CFDINominaRecibo.RID,CFDINominaRecibo:CFDINominaRecibo.Personal,EstacionTrabajo )<BR><BR>Fin<BR><BR>Si<BR> sql(<T>SELECT    CASE    WHEN count(id) <= 0 THEN 1  else  0  END FROM ListaModuloID where Estacion = :testacion <T>, EstacionTrabajo )<BR>Entonces<BR>  Asigna(Temp.texto,<T>PROCESO CONCLUIDO<T>)<BR>informacion(temp.texto)<BR>Sino<BR>  Asigna(Temp.texto,<T><T>)                                                <BR><BR>Fin
