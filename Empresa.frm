...
ExpresionesAlMostrar=Asigna(Info.Tipo,<T>Empresa<T>)
...
EnMenu=S
TipoAccion=Expresion
Expresion=Asigna(Info.Tabla, <T>Empresa<T>)<BR>Asigna(Info.Llave, Carpeta.LlavePrimaria(<T>Ficha<T>, <T>|<T>))<BR>Si(Version.RegHist y (SQL(<T>SELECT Estatus FROM CfgRegHist WHERE SysTabla=:tTabla<T>, Info.Tabla)=EstatusActivo), Forma(<T>RegHist<T>))
Activo=S
ConCondicion=S
EjecucionCondicion=ConDatos(Empresa:Empresa.Empresa)
Antes=S
...
[Detalle.Empresa.Direccion1]
Carpeta=Detalle
Clave=Empresa.Direccion1
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
Empresa.FiscalZona=Empresa.TipoContribuyente
Empresa.TipoContribuyente=Empresa.Representante
...
[Fiscal.Empresa.TipoContribuyente]
Carpeta=Fiscal
Clave=Empresa.TipoContribuyente
Editar=S
ValidaNombre=S
3D=S
Tamano=10
...
Empresa.DireccionNumeroInt=Empresa.Direccion1
Empresa.Direccion1=Empresa.Delegacion
...
[Interes.Empresa.DiasGracia]
Carpeta=Interes
Clave=Empresa.DiasGracia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
[Interes.Empresa.InteresMensual]
Carpeta=Interes
Clave=Empresa.InteresMensual
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
...
(Inicio)=Empresa.DiasGracia
Empresa.DiasGracia=Empresa.InteresMensual
Empresa.InteresMensual=(Fin)
