
[Forma]
Clave=CuotasContrato
Icono=14
Modulos=(Todos)
Nombre=Cuotas

CarpetaPrincipal=CuotasContrato
PosicionInicialAlturaCliente=493
PosicionInicialAncho=1257


ListaCarpetas=CuotasContrato
PosicionInicialIzquierda=25
PosicionInicialArriba=83
BarraHerramientas=S
AccionesTamanoBoton=15x5
AccionesDerecha=S
ListaAcciones=(Lista)
[CuotasContrato.IDContrato]
Carpeta=CuotasContrato
Clave=IDContrato
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.NoCuota]
Carpeta=CuotasContrato
Clave=NoCuota
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.Periodo]
Carpeta=CuotasContrato
Clave=Periodo
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.Ejercicio]
Carpeta=CuotasContrato
Clave=Ejercicio
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.TotalCuota]
Carpeta=CuotasContrato
Clave=TotalCuota
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.Articulo]
Carpeta=CuotasContrato
Clave=Articulo
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CuotasContrato.Precio]
Carpeta=CuotasContrato
Clave=Precio
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.Columnas]
IDContrato=70
NoCuota=69
Periodo=64
Ejercicio=61
TotalCuota=74
Articulo=81
Precio=64

Descripcion1=163
Mov=85
FechaEmision=89
Estatus=80
IDVenta=64
Deposito=155
[CuotasContrato]
Estilo=Hoja
Clave=CuotasContrato
PermiteEditar=S
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=A1
Vista=CuotasContrato
Fuente={Tahoma, 8, Negro, []}
HojaTitulos=S
HojaMostrarColumnas=S
HojaMostrarRenglones=S
HojaColoresPorEstatus=S
HojaPermiteEliminar=S
HojaVistaOmision=Automática
CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)

CarpetaVisible=S
HojaTitulosEnBold=S

Filtros=S
FiltroPredefinido=S
FiltroNullNombre=(sin clasificar)
FiltroEnOrden=S
FiltroTodoNombre=(Todo)
FiltroAncho=20
FiltroRespetar=S
FiltroTipo=Múltiple (por Grupos)
BusquedaRapidaControles=S
FiltroModificarEstatus=S
FiltroCambiarPeriodo=S
FiltroBuscarEn=S
FiltroFechasCambiar=S
FiltroFechasNormal=S
FiltroFechasNombre=&Fecha
FiltroFechas=S
FiltroFechasCampo=CuotasContrato.FechaCuota
FiltroGrupo1=Art.Descripcion1
FiltroValida1=Art.Descripcion1
FiltroTodo=S
OtroOrden=S
ListaOrden=CuotasContrato.NoCuota<TAB>(Acendente)
FiltroGeneral=CuotasContratoD.Precio >=0<BR>AND<BR>CuotasContrato.IDContrato={Info.ID}
[CuotasContrato.CuotasContrato.IDContrato]
Carpeta=CuotasContrato
Clave=CuotasContrato.IDContrato
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.CuotasContrato.NoCuota]
Carpeta=CuotasContrato
Clave=CuotasContrato.NoCuota
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.CuotasContrato.Periodo]
Carpeta=CuotasContrato
Clave=CuotasContrato.Periodo
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.CuotasContrato.Ejercicio]
Carpeta=CuotasContrato
Clave=CuotasContrato.Ejercicio
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.CuotasContrato.TotalCuota]
Carpeta=CuotasContrato
Clave=CuotasContrato.TotalCuota
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.CuotasContratoD.Articulo]
Carpeta=CuotasContrato
Clave=CuotasContratoD.Articulo
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CuotasContrato.CuotasContratoD.Precio]
Carpeta=CuotasContrato
Clave=CuotasContratoD.Precio
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco


[CuotasContrato.Art.Descripcion1]
Carpeta=CuotasContrato
Clave=Art.Descripcion1
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco




[Acciones.Cerrar]
Nombre=Cerrar
Boton=23
NombreEnBoton=S
NombreDesplegar=Cerrar
GuardarAntes=S
EnBarraHerramientas=S
TipoAccion=Ventana
ClaveAccion=Cerrar
Activo=S
Visible=S


EspacioPrevio=S
[CuotasContrato.Mov]
Carpeta=CuotasContrato
Clave=Mov
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=40
ColorFondo=Blanco

[CuotasContrato.FechaEmision]
Carpeta=CuotasContrato
Clave=FechaEmision
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CuotasContrato.Estatus]
Carpeta=CuotasContrato
Clave=Estatus
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=15
ColorFondo=Blanco










[Acciones.Guardar Cambios]
Nombre=Guardar Cambios
Boton=3
NombreEnBoton=S
NombreDesplegar=Guardar Cambios
EnBarraHerramientas=S
TipoAccion=Controles Captura
ClaveAccion=Guardar Cambios
Activo=S
Visible=S

































[CuotasContrato.CuotasContrato.Deposito]
Carpeta=CuotasContrato
Clave=CuotasContrato.Deposito
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco















[Acciones.ModC]
Nombre=ModC
Boton=11
NombreEnBoton=S
NombreDesplegar=Modificar Cuota
EnBarraHerramientas=S
EspacioPrevio=S
TipoAccion=Formas
ClaveAccion=SVModCuota
Activo=S
Antes=S
AntesExpresiones=Asigna(Info.ID,CuotasContrato:CuotasContrato.IDContrato)
Visible=S








[CuotasContrato.ListaEnCaptura]
(Inicio)=CuotasContrato.IDContrato
CuotasContrato.IDContrato=CuotasContrato.NoCuota
CuotasContrato.NoCuota=CuotasContrato.Periodo
CuotasContrato.Periodo=CuotasContrato.Ejercicio
CuotasContrato.Ejercicio=CuotasContrato.TotalCuota
CuotasContrato.TotalCuota=CuotasContratoD.Articulo
CuotasContratoD.Articulo=Art.Descripcion1
Art.Descripcion1=CuotasContratoD.Precio
CuotasContratoD.Precio=CuotasContrato.Deposito
CuotasContrato.Deposito=Mov
Mov=FechaEmision
FechaEmision=Estatus
Estatus=(Fin)










[Acciones.Actualizar]
Nombre=Actualizar
Boton=82
NombreDesplegar=Actualizar
EnBarraHerramientas=S
TipoAccion=Expresion
Activo=S
Visible=S



NombreEnBoton=S


EspacioPrevio=S
Expresion=Forma.ActualizarForma




[Forma.ListaAcciones]
(Inicio)=Guardar Cambios
Guardar Cambios=Actualizar
Actualizar=ModC
ModC=Cerrar
Cerrar=(Fin)
